...
class ImagePredictor {
    ...
    static func createImageClassifier() -> VNCoreMLModel {
        ...
        let imageClassifierWrapper = try? ClassificationModel(configuration: defaultConfig)
    ...
    private func visionRequestHandler(_ request: VNRequest, error: Error?) {
        ...
        for observation in observations {
            ...
            var output = [Float32()]
            guard let arrOutput = observation.featureValue.multiArrayValue else {
                print("Observation produced the wrong result type: \(type(of: observation.featureValue.multiArrayValue)).")
                return
            }
            
            if let floatOutput = try? UnsafeBufferPointer<Float32>(arrOutput) {
              output = Array(floatOutput)
            }
            
            guard let maxConfidence = output.max() else {
                print("Maximum could not be found")
                return
            }
            
            
            guard let classification = categories[output.firstIndex(of: maxConfidence)!] else {
                print("Category could not be determined")
                return
            }
            
            predictions = [Prediction(classification: classification, confidencePercentage: String(maxConfidence))]
            print(predictions!)
        }
